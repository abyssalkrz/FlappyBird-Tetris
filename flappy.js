...
const charactersList = [
    { id: 'player.jpg', name: 'Classic Red', color: '#ff5722', emoji: '🔴' },
    { id: 'player2.jpg', name: 'Aqua Teal', color: '#00bcd4', emoji: '🔵' },
    { id: 'player3.jpg', name: 'Emerald Green', color: '#4caf50', emoji: '🟢' },
    { id: 'player4.jpg', name: 'Retro Gold', color: '#ffeb3b', emoji: '🟡' },
    { id: 'player5.jpg', name: 'Cosmic Violet', color: '#9c27b0', emoji: '🟣' }
];
...
// Ground properties matching retro screenshots
let groundHeight = 56;
...
function renderCharacterScreen() {
    ...
    charactersList.forEach(char => {
        ...
        card.innerHTML = `
            <div class="char-fallback-circle" style="background-color: ${char.color}; border-color: ${char.color}">
...
function renderWheel() {
    ...
    charactersList.forEach((char, index) => {
        ...
        slice.innerHTML = `<span class="wheel-slice-fallback" style="transform: rotate(${-index * angleStep}deg);">${char.emoji}</span>`;
...
function drawFlappy() {
    ...
    pipes.forEach(pipe => {
        ...
        // Lower Pipe Lip Highlight
        flappyCtx.fillRect(pipe.x + 3, lowerY, 6, pipe.bottom);
    });

    // Tan Ground Layer & Green Grass Border Strip
    ...
    // Render Bird Character (Image fallback draws a pixelated vector bird of selected color)
    const activeChar = charactersList.find(c => c.id === selectedChar) || charactersList[0];
    
    flappyCtx.save();
    flappyCtx.translate(bird.x, bird.y);
    
    // Tiny bird angle based on velocity
